...
export function Hero() {
  return (
    <section id="hero" className="relative overflow-hidden px-4 pb-20 pt-8 sm:px-6 lg:px-8 lg:pb-28 lg:pt-12">
      ...
      <motion.div initial="hidden" animate="show" variants={container} className="mx-auto max-w-6xl text-center">
        <motion.div variants={item} className="mx-auto inline-flex items-center gap-2 rounded-full border border-violet-200 bg-white/85 px-4 py-2 text-sm font-semibold text-violet-800 shadow-sm backdrop-blur">
          <Star className="h-4 w-4" />
          {brand.promise}
        </motion.div>

        <motion.h1 variants={item} className="mx-auto mt-6 max-w-4xl text-balance text-5xl font-extrabold tracking-[-0.03em] text-ink sm:text-6xl md:text-7xl lg:text-8xl">
          Authentic Nigerian flavors, delivered to your door.
        </motion.h1>

        <motion.p variants={item} className="mx-auto mt-6 max-w-2xl text-lg leading-8 text-zinc-600 sm:text-xl">
        ...
        <motion.div variants={item} className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
          <motion.a
            ...
            whileTap={{ scale: 0.98 }}
            transition={spring}
            className="inline-flex items-center gap-2 rounded-full bg-violet-800 px-6 py-3.5 text-sm font-semibold text-white shadow-[0_20px_70px_rgba(107,33,168,0.24)]"
          >
          ...
          <motion.a
            ...
            whileTap={{ scale: 0.98 }}
            transition={spring}
            className="inline-flex items-center gap-2 rounded-full border border-violet-200 bg-white px-6 py-3.5 text-sm font-semibold text-violet-800"
          >
            Shop the Collection <ArrowRight className="h-4 w-4" />
          </motion.a>
        </motion.div>

        <motion.div variants={item} className="mt-6 flex flex-wrap items-center justify-center gap-2 text-sm text-zinc-500">
        ...
        <motion.div variants={item} className="relative mx-auto mt-12 max-w-5xl">
