        ...
        <div className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: '-10% 0px' }} variants={reveal} transition={transition} className="max-w-2xl">
            ...
            <motion.a
              ...
              whileTap={{ scale: 0.98 }}
              transition={spring}
              className="mt-7 inline-flex items-center gap-2 rounded-full bg-violet-800 px-6 py-3.5 text-sm font-semibold text-white shadow-[0_20px_70px_rgba(107,33,168,0.2)]"
            >
          ...
          <motion.div
            ...
            className="rounded-[2rem] border border-violet-100 bg-white p-4 shadow-[0_20px_70px_rgba(111,57,231,0.08)]"
          >
            <img src={featuredImage.src} alt={featuredImage.alt} className="h-[18rem] w-full rounded-[1.5rem] object-cover sm:h-[24rem]" loading="lazy" decoding="async" />
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {onePouchHighlights.map((item) => (
                <div key={item.title} className="rounded-[1.25rem] border border-violet-100 bg-white p-4">
